...
all: sitebuild sitebuild-darwin-amd64

sitebuild:
	@go build -o $(OUTPUT_DIR)/sitebuild -ldflags "-X 'main.version=${VERSION}' -X 'main.gitCommit=${GIT_COMMIT}'" ./main.go

sitebuild-darwin-amd64:
	@GOOS=darwin GOARCH=amd64 go build -o $(OUTPUT_DIR)/sitebuild-darwin-amd64 -ldflags "-X 'main.version=${VERSION}' -X 'main.gitCommit=${GIT_COMMIT}'" ./main.go
